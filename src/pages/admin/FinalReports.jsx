...
import { FileText, Eye, Edit3, Trash2, Upload, X, ChevronLeft, ChevronRight, ExternalLink } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { SkeletonTableRow } from '../../components/Skeleton';

const PAGE_SIZE = 10;

export default function AdminFinalReports() {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (uploadFile) {
                ...
                const path = `final_reports/${uploadStudentId}_akhir_${Date.now()}.${ext}`;
            ...
            if (editingReport) {
                const { error } = await supabase.from('final_reports')
                    .update({ student_id: uploadStudentId, status: uploadStatus, file_url: filePath })
                ...
                const { error } = await supabase.from('final_reports')
                    .insert({ student_id: uploadStudentId, status: uploadStatus, file_url: filePath });
                if (error) throw error;
    ...
    return (
        <div>
            <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div>
                    <h1 style={{ fontSize: '1.8rem', marginBottom: '8px' }}>Laporan Akhir Mahasiswa</h1>
                    <p style={{ color: 'var(--text-muted)' }}>Kelola unggahan laporan PDF akhir seluruh mahasiswa magang.</p>
                </div>
            ...
            <div className="glass-panel" style={{ backgroundColor: 'white', overflow: 'hidden' }}>
                {loading ? (
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <tbody>{Array.from({ length: PAGE_SIZE }).map((_, i) => <SkeletonTableRow key={i} cols={5} />)}</tbody>
                    </table>
                ) : reports.length === 0 ? (
                    <div style={{ padding: '60px', textAlign: 'center', color: 'var(--text-muted)' }}>
                        <FileText size={40} strokeWidth={1.2} style={{ margin: '0 auto 12px' }} />
                        <p style={{ margin: 0 }}>Belum ada laporan akhir yang diunggah.</p>
                    </div>
                ) : (
                    <>
                            ...
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '600px' }}>
                                <thead>
                                    <tr style={{ backgroundColor: '#F8FAFC', borderBottom: '1px solid var(--border)' }}>
                                        {['Mahasiswa', 'Status', 'Tanggal Unggah', 'File', 'Aksi'].map(h => (
                        ...
                        {reports.length > PAGE_SIZE && (
                            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderTop:'1px solid var(--border)',backgroundColor:'#FAFAFA' }}>
                                <span style={{ fontSize:'0.85rem',color:'var(--text-muted)' }}>{(page-1)*PAGE_SIZE+1}–{Math.min(page*PAGE_SIZE,reports.length)} dari {reports.length}</span>
                                <div style={{ display:'flex',gap:'6px' }}>
            ...
            {showModal && (
                <div style={{ position:'fixed',inset:0,backgroundColor:'rgba(0,0,0,0.5)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px' }}>
                    <div className="glass-panel" style={{ width:'100%',maxWidth:'480px',backgroundColor:'white',padding:'32px' }}>
                        <h2 style={{ marginBottom:'20px' }}>{editingReport ? 'Edit Laporan Akhir' : 'Unggah Laporan Akhir'}</h2>
            ...
            {deleteConfirm && (
                <div style={{ position:'fixed',inset:0,backgroundColor:'rgba(0,0,0,0.5)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px' }}>
                    <div className="glass-panel" style={{ width:'100%',maxWidth:'400px',backgroundColor:'white',padding:'32px',textAlign:'center' }}>
                        ...
                        <h3 style={{ marginBottom:'8px' }}>Hapus Laporan Akhir?</h3>
